...
#nullable disable
namespace DiscordChatGPTBot.Command.Help
{
    public class Help : TopLevelModule<HelpService>
    {
        ...
        public async Task Modules()
        {
            await ReplyAsync("", false, new EmbedBuilder().WithOkColor().WithTitle("模組清單")
                ...
                .WithFooter("輸入 `c!Commands 模組名稱` 以顯示模組內全部的指令，例 `c!Commands Help`")
                .Build());
        }
        ...
        public async Task Commands([Summary("模組名稱")] string module = null)
        {
            ...
            var embed = new EmbedBuilder().WithOkColor().WithTitle($"{cmds.First().Module.Name} 內包含的指令").WithFooter("輸入 `c!Help 指令` 以顯示指令的詳細說明，例 `c!Help Help`");
        ...
        public async Task H([Summary("指令名稱")] string command = null)
        {
            ...
            {
                EmbedBuilder embed = new EmbedBuilder().WithOkColor().WithFooter("輸入 `c!Modules` 取得所有的模組");
                embed.Title = "聊天小幫手 建置版本" + Program.VERSION;
#if DEBUG
                embed.Title += " (測試版)";
#endif
